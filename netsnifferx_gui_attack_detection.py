...
Versão: GUI com Detecção de Ataques (Port Scan, Brute Force [basic], Flood)

Baseado em: netsnifferx_gui_alerts_usability.py

NOTA IMPORTANTE: Este script requer privilégios de administrador para capturar pacotes.
Execute com: python netsnifferx_gui_attack_detection.py
"""
...
DEFAULT_CONFIG = {
    ...
    "brute_force_detection": {
        ...
        "threshold_failed_attempts": 5, # Tentativas falhas (SYN->RST) em X segundos
...
# Mapeamento de tipos ICMP (igual anterior)
ICMP_TYPES = {
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, "r") as f:
                config = json.load(f)
                # Mesclar com padrão para garantir novas chaves
                updated_config = DEFAULT_CONFIG.copy()
                for key, value in config.items():
                    if key in updated_config and isinstance(updated_config[key], dict) and isinstance(value, dict):
                        updated_config[key].update(value) # Mescla dicionários internos
                    else:
...
class AttackDetector:
    ...
    def update_config(self, new_config):
        ...
        # Limpar trackers pode ser necessário se as janelas de tempo mudarem muito
    ...
    def process_packet(self, packet):
        ...
        if TCP in packet:
            ...
            # Poderia adicionar checagem de UDP scan aqui também
        ...
        # Limpeza periódica (poderia ser em thread separada)
        # self.cleanup_trackers(current_time) # Comentado para teste de performance
    ...
    def check_brute_force(self, current_time, src_ip, dst_ip, dst_port, flags):
        ...
        # Detectar tentativa falha (SYN seguido por RST do servidor, ou apenas SYN sem resposta, ou SYN->SYN/ACK->RST)
        # Simplificação: Contar SYNs para a porta monitorada que não estabelecem conexão (difícil sem state tracking completo)
        # Abordagem mais simples: Contar RSTs vindo do servidor para a porta após um SYN nosso?
        # Abordagem ainda mais simples (usada aqui): Contar SYNs para a porta monitorada. É impreciso mas um ponto de partida.
        is_failed_attempt = (flags & 0x02) # Contando SYNs como tentativas (muito básico)
        # Ideal seria rastrear o estado da conexão TCP para ver se falhou
        ...
        if is_failed_attempt:
            ...
            # Filtra timestamps na janela
            attempts_in_window = [t for t in self.brute_force_tracker[src_ip][dst_port] if current_time - t <= window]
            self.brute_force_tracker[src_ip][dst_port] = attempts_in_window # Mantém apenas os recentes
    ...
    def check_flood(self, current_time, dst_ip, packet_type):
        ...
        self.flood_tracker[dst_ip][protocol_key] = packets_in_window
    ...
    def cleanup_trackers(self, current_time):
        ...
        # Limpeza do Brute Force Tracker (já feita na checagem)
        
        # Limpeza do Flood Tracker (já feita na checagem)
        pass # A limpeza já remove timestamps antigos
...
# ... (código das classes omitido por brevidade, mas deve ser incluído aqui) ...
...
class NetSnifferXGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("NetSnifferX - Monitoramento com Detecção de Ataques") 
        ...
        self.style.configure("Header.TLabel", font=("Arial", 10, "bold")) # Reduzir header da lista
    ...
    def create_widgets(self):
        ...
        display_frame = ttk.LabelFrame(top_panel_frame, text="Tráfego de Rede Detectado (TCP)", padding="10")
        ...
        ttk.Label(header_frame, text="Status", width=15, style="Header.TLabel").pack(side=tk.LEFT, padx=(0, 2))
        ttk.Label(header_frame, text="Duração (s)", width=10, style="Header.TLabel").pack(side=tk.LEFT, padx=(0, 2))
        ...
        legend_frame = ttk.LabelFrame(top_panel_frame, text="Legenda TCP", padding="5")
        legend_frame.pack(fill=tk.X, pady=(10, 0))
        ttk.Label(legend_frame, text="Iniciando", foreground="blue").pack(side=tk.LEFT, padx=(0, 10))
        ttk.Label(legend_frame, text="Ativa", foreground="black").pack(side=tk.LEFT, padx=(0, 10))
        ttk.Label(legend_frame, text="Finalizada", foreground="green").pack(side=tk.LEFT, padx=(0, 10))
        ttk.Label(legend_frame, text="Suspeita", foreground="red").pack(side=tk.LEFT, padx=(0, 10))
        tip_label = ttk.Label(legend_frame, text="Dica: Clique duplo em Suspeita para relatório", font=("Arial", 8, "italic"))
    ...
    def start_capture(self):
        ...
        self.tracker = ConnectionTracker(self.config)
        self.attack_detector.update_config(self.config) # Garante que detector use config atual
        self.tcp_connection_count = 0
        self.suspicious_count = 0
        self.alert_count = 0
        ...
        try:
            while not self.packet_queue.empty():
                ...
                if msg_type == "RAW_PACKET":
                    ...
                    # 1. Processar para Detecção de Ataques (sempre)
                    ...
                    if TCP in packet:
                        ...
                        if conn_info:
                            ...
                            if conn_info.get('trigger_alert', False):
                                self.trigger_tcp_alert(conn_info) # Renomeado para clareza
                                
                    # Exibir informações de tráfego TCP
                    if display_info:
                        self.display_tcp_traffic(display_info)
                        self.update_stats()
            ...
            self.root.after(50, self.process_packet_queue) # Reduzir um pouco o delay?
    
    def display_tcp_traffic(self, info):
        """Exibe APENAS informações de tráfego TCP na interface principal."""
        if info['protocol'] != 'TCP': return # Garantia
        ...
        line = f"{timestamp:<9} {src_ip}:{src_port:<22} {dst_ip}:{dst_port:<22} {status:<15} {duration:<10.2f} {pkts:<7}\n"
    ...
    def trigger_attack_alert(self, attack_type, source_info, details, target_info):
        ...
        # Alerta visual/sonoro para ataques (simplificado por enquanto)
        if self.enable_visual_alerts_var.get(): # Usar o mesmo controle por ora?
            ...
            # Usar showinfo para não pausar a captura como showwarning faz?
        ...
        if self.config["enable_sound_alerts"] and winsound: # Usar o mesmo controle?
            try:
...
class AttackConfigWindow:
    ...
    def create_widgets(self):
        ...
        ttk.Checkbutton(main_frame, text="Habilitar Detecção de Ataques (Geral)", variable=self.attack_detection_enabled).pack(anchor=tk.W, pady=(0, 10))
