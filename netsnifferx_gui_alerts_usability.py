...
Versão: GUI com Alertas Configuráveis e Melhorias de Usabilidade

NOTA IMPORTANTE: Este script requer privilégios de administrador para capturar pacotes.
Execute com: python netsnifferx_gui_alerts_usability.py
"""
...
class ConnectionTracker:
    ...
    def track_packet(self, packet):
        ...
        if flags & 0x02:  # SYN flag
            ...
            # Pacote não SYN/FIN/RST para uma conexão não rastreada - pode acontecer
            # Podemos optar por iniciar o rastreamento aqui ou ignorar
            # Iniciando o rastreamento para capturar conexões já existentes:
            self.connections[conn_id] = {
                'start_time': current_time, # Nota: Tempo de início pode não ser preciso
                'last_update': current_time,
                'status': 'EM ANDAMENTO',
                'packets': 1,
                'flags_history': [flags],
    ...
    def get_connection_details(self, conn_id):
        ...
        if conn_id in self.connections:
            ...
            # Se a conexão não está mais ativa, mas temos histórico
            ...
            details = {
                ...
                'status': 'FINALIZADA (Histórico)', # Indica que foi reconstruído do histórico
                'duration': duration,
...
class NetSnifferXGUI:
    def __init__(self, root):
        ...
        self.connections = {}
        self.alert_count = 0
        self.alert_popup_active = False # Flag para controlar pop-up ativo
        ...
        self.enable_visual_alerts_var = tk.BooleanVar(value=self.config["enable_visual_alerts"]) # Variável para checkbox
    ...
    def create_widgets(self):
        ...
        # Checkbox para ativar/desativar alertas pop-up
        self.visual_alert_check = ttk.Checkbutton(button_frame, text="Alertas Pop-up", variable=self.enable_visual_alerts_var, command=self.toggle_visual_alerts)
    ...
    def open_alert_config(self):
        ...
        # Passar a variável do checkbox para a janela de config também
    ...
    def save_alert_config(self, new_config):
        ...
        # Atualizar a variável do checkbox principal com o valor salvo na janela de config
    ...
    def toggle_visual_alerts(self):
        """Atualiza a configuração quando o checkbox é clicado."""
        self.config["enable_visual_alerts"] = self.enable_visual_alerts_var.get()
        save_config(self.config) # Salvar imediatamente
        
    def start_capture(self):
        if self.is_capturing:
            return
        
        # Atualizar config com valores da UI principal antes de iniciar
        ...
        self.config["enable_visual_alerts"] = self.enable_visual_alerts_var.get() # Incluir estado do checkbox
    ...
    def packet_handler(self, packet):
        # Adiciona pacotes à fila para processamento na thread principal
        # Evita processamento pesado na thread de captura do Scapy
        if TCP in packet:
        ...
        try:
            while not self.packet_queue.empty():
                ...
                if msg_type == "RAW_PACKET":
                    # Processar pacote aqui na thread principal
                    # Verificar se um pop-up está ativo ANTES de processar
                    if self.alert_popup_active:
                        # Se um pop-up está ativo, colocar o pacote de volta na fila
                        # ou simplesmente descartar para evitar acúmulo?
                        # Colocar de volta pode causar loop se a fila encher.
                        # Melhor descartar ou ter uma fila secundária.
                        # Por simplicidade, vamos descartar por enquanto.
                        # print("Pop-up ativo, descartando pacote da fila.")
                        # Ou colocar de volta:
                        # self.packet_queue.put(("RAW_PACKET", data))
                        # time.sleep(0.1) # Pequena pausa para não sobrecarregar
                        # Melhor abordagem: não processar se pop-up ativo
                        continue # Pula para o próximo item da fila
                        
                    conn_info = self.tracker.track_packet(data)
                    if conn_info:
                        self.display_connection(conn_info)
                        # Atualizar contadores apenas se for uma nova conexão ou estado mudou?
                        # Lógica atual atualiza a cada pacote processado.
                        # Vamos manter assim por enquanto.
                        self.connection_count += 1 # Ou basear no estado? Melhor contar conexões únicas.
                        if conn_info['is_suspicious']:
                            # Contar apenas uma vez por conexão suspeita?
                            if conn_info['id'] not in self.connections or not self.connections[conn_info['id']]['is_suspicious']:
                                self.suspicious_count += 1
                        self.update_stats()
                        self.connections[conn_info['id']] = conn_info # Atualiza ou adiciona
                    ...
                    if data == "Pronto":
                        ...
                        self.stop_button.config(state=tk.DISABLED) # Garantir que stop está desabilitado
                        
        except queue.Empty:
            pass
        finally:
            self.root.after(100, self.process_packet_queue) # Agendar próxima verificação
    ...
    def display_connection(self, conn_info):
        ...
        if conn_info['is_suspicious']:
            ...
            color = "blue"
        elif status == 'FINALIZADA' or status == 'FINALIZADA (Histórico)':
            color = "green"
        else:
            color = "black"
        ...
        # Lógica para atualizar linha existente ou adicionar nova?
        # Por simplicidade, apenas adiciona no final.
    ...
    def update_stats(self):
        # Recalcular contadores com base no estado atual de self.connections
        active_connections = len(self.tracker.connections)
        suspicious_now = sum(1 for conn in self.tracker.connections.values() if conn['status'] == 'SUSPEITA')
        # Ou usar o histórico self.connections da GUI?
        # Vamos usar o tracker que é mais preciso
        # self.conn_count_label.config(text=f"Conexões Ativas: {active_connections}") # Contar ativas
        self.conn_count_label.config(text=f"Conexões Total: {len(self.connections)}") # Contar total exibido
        self.suspicious_label.config(text=f"Suspeitas: {self.suspicious_count}") # Contar total de suspeitas detectadas
    ...
    def clear_display(self):
        ...
        self.alert_count = 0
        self.connections = {}
        self.tracker = ConnectionTracker(self.config) # Reiniciar tracker também
        self.update_stats()
    ...
    def on_connection_click(self, event):
        try:
            ...
            # Tentar obter detalhes da conexão (ativa ou do histórico)
            ...
            if conn_details and conn_details['is_suspicious']:
                ...
                messagebox.showinfo("Informação", "Detalhes da conexão não encontrados no histórico.")
                
        except Exception as e:
            messagebox.showerror("Erro", f"Erro ao processar clique: {e}")
    
    def trigger_alert(self, conn_info):
        # Verificar se um pop-up já está ativo
        if self.alert_popup_active:
            # print("Alerta pop-up já ativo, ignorando novo alerta visual.")
            # Ainda pode tocar som se habilitado?
            # Decidimos não fazer nada se um pop-up estiver ativo.
            return
        ...
        # Verificar se alertas visuais estão habilitados via checkbox
        if self.enable_visual_alerts_var.get():
            ...
            try:
                # Definir flag ANTES de mostrar o pop-up modal
                self.alert_popup_active = True
                # Mostrar o pop-up (bloqueia a execução até ser fechado)
                messagebox.showwarning(alert_title, alert_message)
            finally:
                # Limpar flag DEPOIS que o pop-up for fechado
                self.alert_popup_active = False
            
            self.flash_alert_label() # Piscar o contador de alertas

        # Alertas sonoros (não bloqueantes, podem tocar mesmo com pop-up ativo?)
        # Vamos manter a lógica original: toca se habilitado na config
    ...
    def flash_alert_label(self, count=5):
        ...
        if count > 0:
            ...
            # Garantir que termine em vermelho se houver alertas
            if self.alert_count > 0:
                 self.alert_label.config(foreground="red")
            else:
                 self.alert_label.config(foreground="black") # Ou cor padrão
    ...
    def on_closing():
        if app.is_capturing:
            if messagebox.askokcancel("Sair", "A captura está em andamento. Deseja realmente sair?"):
                ...
                app.config["enable_visual_alerts"] = app.enable_visual_alerts_var.get() # Usar variável do checkbox
                # Não precisamos salvar enable_sound_alerts aqui, pois é salvo na janela de config
